/**
 * app/api/admin/tickets/[id]/download/route.ts
 * Descarga de boleto por el ADMIN — sin filtro user_id.
 * Reutiliza exactamente el mismo HTML del endpoint de usuario.
 */
...
export async function GET(
  ...
  try {
    ...
    // Obtener boleto SIN filtrar por user_id
    const { data: ticket, error } = await supabaseAdmin
      .from("tickets")
      .select(`
        id, price, status, qr_code, created_at,
        order:orders (id, created_at, total_amount, payment_method),
        match:matches (
    ...
    // Supabase puede retornar la FK order como objeto O como array — normalizamos ambos casos
    const rawOrder = ticket.order
    const orderObj = Array.isArray(rawOrder) ? rawOrder[0] : rawOrder
    const order = orderObj as unknown as { id: string; created_at: string; total_amount: number; payment_method?: string } | null

    // Detectar cortesía: precio $0 en el boleto O payment_method="courtesy" en la orden
    const priceNum = Number(ticket.price)
    const isCourtesy = priceNum === 0 || order?.payment_method === "courtesy"

    console.log(`[Ticket ${id}] price=${ticket.price} priceNum=${priceNum} payment_method=${order?.payment_method} isCourtesy=${isCourtesy}`)
    ...
    const statusLabels: Record<string, string> = {
      activo: "VÁLIDO", usado: "USADO", expirado: "EXPIRADO", cancelado: "CANCELADO",
    }
    const statusColors: Record<string, string> = {
      activo: "#00C853", usado: "#9E9E9E", expirado: "#F44336", cancelado: "#F44336",
    }
...
<body>
    ...
    <div class="ticket-footer">
      ...
      ${isCourtesy
        ? `<div style="display:flex;flex-direction:column;align-items:flex-end;gap:2px">
             <span style="font-size:9px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#B45309">Tipo de boleto</span>
             <div style="display:flex;align-items:center;gap:6px;background:#FEF3C7;border:1.5px solid #F59E0B;border-radius:99px;padding:4px 12px">
               <span style="font-size:13px;font-weight:900;color:#92400E;letter-spacing:1px">CORT&#201;S&#205;A</span>
             </div>
             <span style="font-size:18px;font-weight:900;color:#92400E">$0.00 <span style="font-size:11px;font-weight:600;color:#B45309">MXN</span></span>
           </div>`
  ...
  <div class="actions">
    ...
    console.error("Admin ticket download error:", err)
    return NextResponse.json({ error: "Error al generar el boleto" }, { status: 500 })
  }
}
